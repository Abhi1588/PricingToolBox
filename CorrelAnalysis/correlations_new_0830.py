...
def computeCorrel(borDf,SpreadDf, matu = "All",plot_curve = True):
    ...
    for mat in matu:
        ...
        mean_correl.update({mat:[returns_df.correl.mean(), returns_df['returns_x'].corr(returns_df['returns_y'])]})
        if plot_curve:
    ...
    return pd.DataFrame.from_dict(mean_correl, orient='index', columns= ["Mean_Correl","FullCorrel"])

def computeFXCorrel(borDf,SpreadDf, matu = "All",plot_curve = True):
    window = 7*52
    if matu == 'All':
        matu = set(SpreadDf.Maturity)
    ...
    df_Spreads = SpreadDf[SpreadDf.Date.notna()].copy()
    df_BOR = borDf[borDf.Date.notna()].copy()
    df_BOR.Date = pd.to_datetime(df_BOR.Date)
    mean_correl = {}
    ...
    for mat in matu:
        ...
        mean_correl.update({mat:[returns_df.correl.mean(), returns_df['returns_x'].corr(returns_df['returns_y'])]})
        if plot_curve:
            curve = [str(_) for _ in set(borDf.Currency)][0] + 'USD'
            spread = [str(_) for _ in set(SpreadDf.Curve_x)][0] +' ' +[str(_) for _ in set(SpreadDf.Curve_y)][0]
            plot_curves(returns_df,mat,"{} vs {} Spread".format(curve,spread),True)
...
EIBvsEIBEON.to_csv(r"longterm\LongTermCorrelations_EIBEURvsEIBEON_Spreads.csv")
#LIBUSD swap against ois-Bor spread
LIBUSDvsLIBOIS = computeCorrel(df_LIBUSD,df_Spreads_LIBOIS)
LIBUSDvsLIBOIS.to_csv(r"longterm\LongTermCorrelations_LIBUSDvsUSDOIS_Spreads.csv")
#LIBUSD swap against SOF-bor spread
LIBUSDvsLIBSOF = computeCorrel(df_LIBUSD,df_Spreads_LIBSOF)
LIBUSDvsLIBSOF.to_csv(r"longterm\LongTermCorrelations_LIBUSDvsLIBSOF_Spreads.csv")

#TODO: FX vs Spreads
FXvsEIBEON = computeFXCorrel(df_FX, df_Spreads_EIBEON)
FXvsEIBEON.to_csv(r"longterm\LongTermCorrelations_FXvsEIBEON_Spreads.csv")

FXvsLIBOIS = computeFXCorrel(df_FX, df_Spreads_LIBOIS)
FXvsLIBOIS.to_csv(r"longterm\LongTermCorrelations_FXvsLIBOIS_Spreads.csv")
...
EIBEURvsMulti.to_csv(r"longterm\LongTermCorrelations_EIBEURvsMultiCCY_Spreads.csv")
LIBUSDvsMulti = computeCorrel(df_LIBUSD,df_Multi_Spread)
LIBUSDvsMulti.to_csv(r"longterm\LongTermCorrelations_LIBUSDvsMultiCCY_Spreads.csv")

#TODO: FX vs Spreads
FXvsMulti = computeFXCorrel(df_FX, df_Multi_Spread)
FXvsMulti.to_csv(r"longterm\LongTermCorrelations_FXvsMulti_Spreads.csv")
